...
- **Authenticated R2 Upload** (`server/services/r2Client.ts`): Provides `headR2()` and `getR2Stream()` using AWS SDK S3Client for authenticated Cloudflare R2 access. Publish flow uses `localUploadFromR2()` which streams video from R2 directly to Yodeck as a local upload (POST /media → GET upload_url → PUT stream → complete → poll), avoiding presigned URL HEAD failures entirely.
- **Video Normalization Pipeline** (`normalizeForYodeck` in `videoTranscodeService.ts`): Ensures every ad is optimized for instant Yodeck playback. After upload, automatically normalizes to H.264/yuv420p/faststart/CFR 25fps/keyframe every 2s/no audio. Stores result in `normalizedStoragePath` and metadata in `yodeckMetadataJson`. Publish uses `normalizedStoragePath || convertedStoragePath || storagePath` (best-first). Admin endpoint: `POST /api/admin/media/:assetId/normalize-yodeck`.
- **Media CDN Proxy** (`server/routes/mediaCdn.ts`): Signed-token streaming proxy at `/api/media-cdn/:token` that serves R2 media with full HEAD (200), GET (200), and Range (206) support. Yodeck import uses this CDN URL instead of direct R2 presigned URLs. Tokens encode storagePath, MIME type, filename, and expiry (HMAC SHA256). Publish flow validates CDN HEAD+Range before sending URL to Yodeck, with automatic fallback to local R2 upload if CDN check fails. Debug endpoint: `GET /api/admin/debug/cdn-check?planId=`.
